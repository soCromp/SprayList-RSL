...
echo "#procs,rsl,spray unweighted,lotan_shavit unweighted,linden unweighted,spray weighted,lotan_shavit weighted,linden weighted,spray bimodal,lotan_shavit bimodal,linden bimodal"

for i in "${num_procs[@]}"
#for i in {1..80}
do
  rsl[$i]=`$bin -i $input -o out.txt -n $i -r -m 2000000 | grep $var | grep '(?<= )[0-9]+\.?[0-9]*' -Po`
  ...
  echo $i,${rsl[$i]},${spray_u[$i]},${lotan_shavit_u[$i]},${linden_u[$i]},${spray_w[$i]},${lotan_shavit_w[$i]},${linden_w[$i]},${spray_b[$i]},${lotan_shavit_b[$i]},${linden_b[$i]}
done
