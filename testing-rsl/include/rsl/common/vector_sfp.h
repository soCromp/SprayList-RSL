...
/// SFP: Sorted, Fixed capacity, PERSISTENT (via PTM)
template <typename K, typename V, int CAPACITY> class vector_sfp {
  /// List of keys
  ///
  /// [mfs] I think this still needs to be atomic, because check_next doesn't
  ///       acquire in write mode when reading key_list.
  K key_list[CAPACITY];
  ...
  ///
  /// [mfs] See above: should be atomic!
  int size = 0;
...
public:
  ...
  bool insert_and_steal_greater(vector_sfp *victim, const value_type &pair) {
  ...
  /// This method overwrites the contents of the current vector with the stolen
  /// elements; it is assumed it is called when the current vector is empty.
  void steal_half_and_insert(vector_sfp *victim, const value_type &pair) {
    ...
    if (insert_pos <= median_pos) {
      ...
      TX_RAII; // [mfs] We're about to start writing
               // [mfs] removed thread fence
  ...
  /// This method overwrites the contents of the current vector with the stolen
  /// elements; it is assumed it is called when the current vector is empty.
  void steal_half(vector_sfp *victim) {
    int median_pos = victim->size / 2;
