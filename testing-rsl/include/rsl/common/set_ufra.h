...
/// All fields are relaxed atomic variables.
/// UFRA: Unsorted, Fixed capacity, Relaxed Atomic
template <typename K, int CAPACITY> class set_ufra {
...
public:
  ...
  void clear_and_steal_all(set_ufra<K, CAPACITY> *other) {
    asm volatile("" ::: "memory"); // ensure everything is flushed to RAM
    std::memcpy(key_list, other->key_list, sizeof(rlx_atomic<K>) * other->size);
    asm volatile("" ::: "memory"); // ensure everything is flushed to RAM
  ...
  void copyall(set_ufra<K, CAPACITY> *victim) {
    // Check for overfull
  ...
  /// Consume another set_ufra, stealing all of its elements.
  /// This method assumes the sets are disjoint already.
  void merge(set_ufra<K, CAPACITY> *victim) {
    copyall(victim);
    victim->size = 0;
  }
