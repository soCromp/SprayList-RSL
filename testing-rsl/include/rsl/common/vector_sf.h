...
/// SF: Sorted, Fixed capacity
template <typename K, typename V, int CAPACITY> class vector_sf {
  /// List of keys
  K key_list[CAPACITY];
...
public:
  ...
  bool insert_and_steal_greater(vector_sf *victim,
    ...
    int needed_capacity = entries_to_steal + 1;
  ...
  /// This method overwrites the contents of the current vector with the stolen
  /// elements; it is assumed it is called when the current vector is empty.
  void steal_half_and_insert(vector_sf *victim,
                             const value_type &pair) {
  ...
  /// This method overwrites the contents of the current vector with the stolen
  /// elements; it is assumed it is called when the current vector is empty.
  void steal_half(vector_sf *victim) {
    int median_pos = victim->size / 2;
