...
public:
  ...
  //remove the element at a specific index, shifting
  //elements at greater indices to fill the gap
  bool removeInd(const int64_t ind, K &k, V &v) {
    if(ind > size || ind < 0)
      return false;
  ...
  //returns index from which pair was removed if success, -1 if else
  int remove_pair(K &k, V &v) {
    int ind = -1;
    for(int i = 0; i < size; i++) {
      if(key_list[i] == k && val_list[i] == v) {
        ind = i;
        removeInd(i, k, v);
        return ind;
      }
  ...
  //remove the 0th element in the vector, if that element's
  //key matches that of the key passed in. used to orphanize
  //chunks in the RSL orphanizing process for extract min
  bool orphanize(K k, V &v) {
    assert(k == key_list[0]);
    V _; //dummy argument
    return removeInd(0, k, v);
  }
  ...
  //returns first pair in the vector, putting elems into k and v
  void first_pair(K &k, V &v) {
    k = key_list[0];
    v = val_list[0];
  }
  ...
  void foreach (std::function<void(const K &, V &, bool &)> f,
    ...
    for (int i = 0; i < size && !exit_flag; ++i) {
      // TODO: This is necessary because val_list is atomic,
      ...
      // TODO: If contains() starts locking in DL, the we can just cast here.
      V v = val_list[i].load();
