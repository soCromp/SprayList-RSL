...
///
/// Template Parameters:
/// pkey_t       K         - The type of the key for k/v pairs
/// pval_t       V         - The type of the value for k/v pairs
...
class rsl {
    ...
    static int LAYERS; //num index layers
    ...
    template <typename T, int EXP> struct node_t {
        ...
        vector_sfra<pkey_t, T, get_vector_size()> v;
        ...
        bool insert(const std::pair<const pkey_t, T> &pair) {
            bool overfull = false;
  ...
  typedef node_t<pval_t, data_exp> data_t;
  ...
  static thread_local thread_hp_context_t *hp_context;

  // The head of the global linked list of contexts.
  static std::atomic<thread_hp_context_t *> contexts_head;
  ...
  template <typename T>
  bool check_next(T *&curr, uint64_t &curr_lock, const pkey_t &k, bool cleanup) {
  ...
  template <typename T>
  bool follow(index_t *curr, uint64_t &curr_lock, const pkey_t &k, T *&down) {
  ...
  data_t *skip_to(const pkey_t &k) {
    ...
    top:
      ...
      if (!curr_dl->lock.upgrade(curr_lock)) {
          // Sequence lock check failed
          hp_context->drop_all();
          goto top;
      }
      ...
      return curr_dl;
  } //ends skip_to on line 644

  bool orphanize(const pkey_t &k) {
      init_context();
      top:
...
public:
  ...
  typedef std::pair<const pkey_t, pval_t> value_type;
  ...
  bool contains(const pkey_t &k, pval_t &v) {
